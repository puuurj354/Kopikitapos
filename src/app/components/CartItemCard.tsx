...
export function CartItemCard({ item }: CartItemCardProps) {
  ...
  return (
    ...
    >
      <div className="w-10 h-10 bg-[#F5E6D3] rounded-lg flex items-center justify-center text-xl">
        {item.emoji}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-[#3E2723] truncate">{item.name}</p>
        <p className="text-xs text-[#8B5E3C]">Rp {item.price.toLocaleString()}</p>
      </div>
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-medium text-[#3E2723] w-5 text-center">{item.quantity}</span>
        <button
          onClick={() => updateQuantity(item.id, item.quantity + 1)}
