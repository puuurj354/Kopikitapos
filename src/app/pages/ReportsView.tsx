...
export function ReportsView() {
  ...
  const chartData =
    period === 'Harian'
      ? hourlyData.map((h) => ({ name: h.hour, sales: h.sales, orders: Math.round(h.sales / 35000) }))
      : period === 'Mingguan'
      ? weeklySales
      : monthlyData;
  ...
  const completionRate = Math.round((completedOrders / recentOrders.length) * 100);

  const tooltipStyle = { backgroundColor: '#3E2723', border: 'none', borderRadius: '12px', color: '#fff' };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-[#3E2723]">Laporan Keuangan</h1>
          <p className="text-[#8B5E3C] mt-1">Analisis pendapatan dan performa penjualan</p>
        </div>
          ...
          {PERIODS.map((p) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                period === p ? 'bg-[#6F4E37] text-white shadow-sm' : 'text-[#8B5E3C] hover:bg-[#F5E6D3]'
              }`}
            >
              {p}
            </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: '💰', label: 'Total Pendapatan', value: `Rp ${(totalRevenue / 1000000).toFixed(1)}jt`, change: '+12%', up: true },
          { icon: '📦', label: 'Total Pesanan', value: String(totalOrders), change: '+8%', up: true },
          { icon: '✅', label: 'Tingkat Selesai', value: `${completionRate}%`, change: '+3%', up: true },
          { icon: '💵', label: 'Rata-rata/Pesanan', value: `Rp ${Math.round(totalRevenue / Math.max(totalOrders, 1)).toLocaleString()}`, change: '+5%', up: true },
        ].map((card, i) => (
          <motion.div
            key={card.label}
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="w-11 h-11 bg-[#F5E6D3] rounded-xl flex items-center justify-center text-2xl">{card.icon}</div>
              <span className={`text-xs font-semibold px-2 py-1 rounded-full ${card.up ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                {card.change}
              </span>
            </div>
            <p className="text-xs text-[#8B5E3C]">{card.label}</p>
            <p className="text-xl font-bold text-[#3E2723] mt-0.5">{card.value}</p>
          </motion.div>
        ))}
      </div>
      ...
      >
        <h3 className="font-bold text-[#3E2723] mb-4">Grafik Pendapatan — {period}</h3>
        <ResponsiveContainer width="100%" height={280}>
          <LineChart id={`reports-line-${period}`} key={`line-${period}`} data={chartData} accessibilityLayer={false}>
            <CartesianGrid strokeDasharray="3 3" stroke="#F5E6D3" />
            <XAxis dataKey="name" stroke="#8B5E3C" tick={{ fontSize: 12 }} />
            <YAxis stroke="#8B5E3C" tickFormatter={(v) => `${(v / 1000000).toFixed(1)}M`} tick={{ fontSize: 12 }} />
            <Tooltip
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart id={`reports-bar-${period}`} key={`bar-${period}`} data={chartData} accessibilityLayer={false}>
              <CartesianGrid strokeDasharray="3 3" stroke="#F5E6D3" />
              <XAxis dataKey="name" stroke="#8B5E3C" tick={{ fontSize: 12 }} />
            ...
            {topItems.map((item, i) => (
              <div key={item.name} className="flex items-center gap-3">
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${
                  i === 0 ? 'bg-amber-400 text-white' : i === 1 ? 'bg-gray-300 text-gray-700' : i === 2 ? 'bg-amber-700 text-white' : 'bg-[#F5E6D3] text-[#8B5E3C]'
                }`}>{i + 1}</span>
                ...
                <div className="text-right shrink-0">
                  <p className="text-xs font-semibold text-[#3E2723]">{item.sold}x</p>
                  <p className="text-xs text-[#A0826D]">Rp {(item.revenue / 1000000).toFixed(1)}jt</p>
                </div>
              </div>
            ))}
          </div>
