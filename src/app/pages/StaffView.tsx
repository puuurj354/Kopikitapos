...
const INITIAL_STAFF: StaffMember[] = [
  { id: 1, name: 'Sari Barista', role: 'barista', shift: 'Shift Pagi (07:00–15:00)', ordersToday: 42, status: 'active' },
  { id: 2, name: 'Budi Kopi', role: 'barista', shift: 'Shift Siang (13:00–21:00)', ordersToday: 18, status: 'active' },
  { id: 3, name: 'Rina Cahya', role: 'barista', shift: 'Shift Pagi (07:00–15:00)', ordersToday: 37, status: 'active' },
  { id: 4, name: 'Joko Prasetyo', role: 'barista', shift: 'Shift Malam (17:00–23:00)', ordersToday: 0, status: 'off' },
  { id: 5, name: 'Admin Manager', role: 'admin', shift: 'Full Time', ordersToday: 0, status: 'active' },
];

const SHIFTS = ['Shift Pagi (07:00–15:00)', 'Shift Siang (13:00–21:00)', 'Shift Malam (17:00–23:00)', 'Full Time'];

export function StaffView() {
  const [staff, setStaff] = useState<StaffMember[]>(INITIAL_STAFF);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ name: '', role: 'barista' as StaffMember['role'], shift: SHIFTS[0] });
  ...
  const toggleStatus = (id: number) => {
    setStaff((prev) =>
      prev.map((s) => (s.id === id ? { ...s, status: s.status === 'active' ? 'off' : 'active' } : s))
    );
  };
  ...
  const handleAddStaff = () => {
    if (!form.name.trim()) { setFormError('Nama harus diisi'); return; }
    ...
    setForm({ name: '', role: 'barista', shift: SHIFTS[0] });
    setShowForm(false);
    setFormError('');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-[#3E2723]">Manajemen Staff</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={card.label} className="bg-white rounded-2xl p-5 border border-[#F5E6D3] shadow-sm">
            <div className="w-10 h-10 bg-[#F5E6D3] rounded-xl flex items-center justify-center text-xl mb-3">{card.icon}</div>
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            <h3 className="font-bold text-[#3E2723] mb-4">➕ Tambah Anggota Staff</h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="text-sm text-[#8B5E3C] mb-1 block">Nama</label>
                <input
                  value={form.name}
              ...
              <div>
                <label className="text-sm text-[#8B5E3C] mb-1 block">Role</label>
                <select
                  value={form.role}
              ...
              <div>
                <label className="text-sm text-[#8B5E3C] mb-1 block">Shift</label>
                <select
                  value={form.shift}
          ...
          <div className="flex gap-3 flex-wrap justify-center">
            {admins.map((a) => (
              <div key={a.id} className="flex flex-col items-center gap-1">
                <div className="w-12 h-12 bg-amber-100 border-2 border-amber-400 rounded-full flex items-center justify-center text-2xl">👑</div>
                <span className="text-xs font-medium text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">Admin</span>
            ...
            {baristas.map((b) => (
              <div key={b.id} className="flex flex-col items-center gap-1">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-2xl border-2 ${
                  b.status === 'active' ? 'bg-[#F5E6D3] border-[#6F4E37]' : 'bg-gray-100 border-gray-300'
                }`}>👤</div>
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                  b.status === 'active' ? 'text-[#6F4E37] bg-[#F5E6D3]' : 'text-gray-500 bg-gray-100'
                }`}>Barista</span>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#F5E6D3]">
                <th className="text-left py-4 px-6 text-[#3E2723] font-semibold text-sm">Nama</th>
                <th className="text-left py-4 px-6 text-[#3E2723] font-semibold text-sm">Role</th>
                <th className="text-left py-4 px-6 text-[#3E2723] font-semibold text-sm">Shift</th>
                <th className="text-left py-4 px-6 text-[#3E2723] font-semibold text-sm">Pesanan Hari Ini</th>
              ...
              <AnimatePresence>
                {staff.map((member) => (
                  ...
                  >
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${
                          member.role === 'admin' ? 'bg-amber-100' : 'bg-[#F5E6D3]'
                        }`}>
                          {member.role === 'admin' ? '👑' : '👤'}
                        </div>
                        <span className="font-medium text-[#3E2723] text-sm">{member.name}</span>
                      </div>
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6 text-sm text-[#8B5E3C]">{member.shift}</td>
                    <td className="py-4 px-6">
                      <span className="font-semibold text-[#3E2723] text-sm">{member.ordersToday}</span>
                      <span className="text-xs text-[#A0826D] ml-1">pesanan</span>
                    </td>
                    <td className="py-4 px-6">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
                    ...
                    <td className="py-4 px-6">
                      <div className="flex gap-2">
                        ...
                        >
                          {member.status === 'active' ? 'Set Off' : 'Aktifkan'}
                        </button>
                        {member.role !== 'admin' && (
                          <button
      ...
      <div className="bg-[#F5E6D3] rounded-xl p-4 flex items-center gap-3">
        ...
        <p className="text-sm text-[#6F4E37]">
          ...
          <span className="font-bold text-[#3E2723]">{totalOrdersToday} pesanan</span>
        </p>
      </div>
    </div>
  );
